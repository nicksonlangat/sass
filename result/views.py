...
from SASS.settings import MEDIA_ROOT, BASE_DIR, STATIC_URL
import os
...
@login_required 
def profile(request):
    """ Show profile of any user that initiates profile request """
    current_semester = Semester.objects.get(is_current_semester=True)
...
def profile_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            messages.success(request, 'You successfully made changes to your profile! ')
            return redirect("/profile/")
    else:
...
def course_registration(request):
    if request.method == 'POST':
        ...
        for s in range(0,len(ids)):
            ...
            obj.save()
            messages.success(request, 'Course(s) Registered Successfully!')
        return redirect('profile')
    else:
        student = Student.objects.get(user__pk=request.user.id)
...
def course_registration_form(request):
  ...
  #  fname = fname.replace("/", "-")
    flocation = '/sass/'+fname
    doc = SimpleDocTemplate(fname, rightMargin=15, leftMargin=15, topMargin=0, bottomMargin=0)
    styles = getSampleStyleSheet()
    ...
    normal.fontSize = 12
    normal.leading = 18
    title = "<b>MOI UNIVERSITY, MAIN CAMPUS</b>" 
    title = Paragraph(title.upper(), normal)
    Story.append(title)
    style = getSampleStyleSheet()
    ...
    school.fontSize = 10
    school.leading = 18
    school_title = "<b>SCHOOL OF ARTS AND SOCIAL SCIENCES </b>"
    ...
    department_title = "<b>DEPARTMENT OF LINGUISTICS AND FOREIGN LANGUAGES</b>"
    ...
    have been duly registered for the <b>" + student.level + " level </b> of study in the department\
    of LINGUISTICS AND FOREIGN LANGUAGES and that the courses and units registered are as approved by the senate of the University"
